...
class KickLiveListener:
    ...
    Captura suscripciones, regalos de subs (gifts) y eventos de chat en directo
    sin necesidad de IP pública ni túneles ngrok.
    """
    ...
    async def get_channel_info(self):
        """Obtiene el ID del canal y del chatroom de Kick."""
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"[!] No se pudo obtener ID directo para @{self.channel_slug} (HTTP {resp.status_code}). Usando fallback...")
                    self.channel_id = abs(hash(self.channel_slug)) % 1000000
                    self.chatroom_id = self.channel_id
                    return True
    ...
    async def start(self):
        ...
        while self.is_running:
            try:
                print(f"[*] Conectando a Kick Pusher WebSocket para @{self.channel_slug}...")
    ...
    async def handle_kick_event(self, event_name: str, payload: dict):
        ...
        try:
            ...
            # 1. Suscripción Regalada Múltiple (Gift Subs: 5, 10, 20, 50 subs)
            if "GiftedSubscriptionsEvent" in event_name or event_name == "App\\Events\\GiftedSubscriptionsEvent":
                username = event_data.get("gifter_username") or event_data.get("username")
                subs_count = int(event_data.get("gifted_user_count", event_data.get("count", 1)))
            ...
            elif "SubscriptionEvent" in event_name or event_name == "App\\Events\\SubscriptionEvent":
            ...
            # Eventos menores de chat ignorados
            pass
